...
export default {
  ...
  raycast(body, layer, { raylength = 30, precision = 0, steepnes = 1 }) {
    ...
    if (hits.length > 0) {
      // some will return true when even one of them satisfy the condition
      hasHit = this.prevHasHit = hits.some((hit) => hit.index !== -1); //stores in the previous and the current
    }
